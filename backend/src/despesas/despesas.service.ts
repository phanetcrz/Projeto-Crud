...
export class DespesasService {
  ...
  formatarDespesa(despesa: CreateDespesaDto | UpdateDespesaDto): DespesaInput {
    const novaDespesa = {
      ...despesa,
    }; /* ...despesa siginifica pegar tudo que tem em despesa */

    if (despesa.data) {
      //**Verifica se existe o campo data e se tem valor */
    ...
    if (despesa.valor) {
      novaDespesa.valor = parseFloat(
        `${despesa.valor}`,
      ); /* Converte o valor para float */
    }

    return novaDespesa as DespesaInput;
  }
    ...
    try {
      ...
      return this.prisma.despesa.create({
        data: {
          ...
          usuario: {
            //--adicionando o ID na criação da despesa
  ...
  async findAll(email: string) {
    try {
      ...
      throw new InternalServerErrorException(
        'Não foi possível obter as despesas. Tente novamente mais tarde.',
      );
    }
  }
  ...
  lancaErro404(e: unknown, stringAcao: string) {   //--unknown é um tipo desconhecido
    ...
    throw new InternalServerErrorException(
      `Não foi possível ${stringAcao}. Tente novamente mais tarde.`,
    );
  }

  async pegaIdPorEmail(email: string) {
    const usuario = await this.prisma.usuario.findUnique({
      //--Pega o email vindo da requisição e faz o filtro por email na tabela do usuário
      where: { email: email },
    });
