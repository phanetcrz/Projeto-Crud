...
export class AuthService {
  ...
  async criar(criarUsuarioDTO: CriarUsuarioDTO) {
    ...
    try {
      return this.prisma.usuario.create({
        ...
        select: {
          //--aqui define que ao executar a criação do usuario, só vai retornar o email do usuário
          nome: true,
          email: true,
        },
      });
  ...
  async logar(logarUsuarioDTO: LogarUsuarioDTO) {
    try {
      ...
      const payload = {
        //aonde é definido o que vai estar dentro do token, que por enquanto será apenas o usuário
        email: usuario.email,
      };
      ...
      throw new InternalServerErrorException(
        'Houve um problema ao logar o seu usuário. Tente novamente mais tarde.',
      );
    }
  }
}
