...
@Catch()
export class FormataFilter<T> implements ExceptionFilter {
  catch(exception: unknown, host: ArgumentsHost) {
    const resposta = host.switchToHttp().getResponse()
    console.log(resposta)

    const ehHttpException = exception instanceof HttpException
    const status = ehHttpException ? exception.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR /* Se a excessão não for do tipo exception então lança um status 500 pelo INTERNAL_SERVER_ERROR*/
